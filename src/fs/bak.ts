...
// writes. M6 added `stash`: the remote version a "keep local" answer overwrites lands here too, so no
// conflict decision destroys the only copy of anything. It lives under `logseq/bak/`, which both Logseq
// (`ignored-path?`) and our scanner ignore.
...
import { assertRelPath, type FileData, type GraphFs } from './graphFs'
...
export interface BakSession {
  ...
  /** Writes content that exists nowhere locally (a remote version about to be overwritten) to `<dir>/<path>`; returns the bak path. */
  stash(path: string, data: FileData): Promise<string>
}
