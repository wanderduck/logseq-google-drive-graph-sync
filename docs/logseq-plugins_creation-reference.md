...
- **Handler facts, read from the shipped 0.10.15 source** (the source map `resources/app/electron.js.map` embeds `electron/handler.cljs`, `electron/utils.js`, `logseq/common/graph.cljs`, `electron/fs_watcher.cljs`; M5, 2026-09-15). `preload.js`: `apis.doAction = (arg) => ipcRenderer.invoke('main', arg)`; `set-ipc-handler!` wraps every call in `try … catch e → e`, so a **synchronous** handler failure resolves with the `Error`, while a handler that returns a promise (`copyFile` = `fs-extra.copy`, `listdir` = async `deepReadDir`) makes `invoke` **reject** with `Error invoking remote method 'main': Error: <code>: …`.
  - `['mkdir-recur', dir]` → `mkdirSync(dir, {recursive: true})`. `['stat', path]` → `{size, mtime, ctime}` (Dates; no `isDirectory`). `['rename', from, to]` → `renameSync`. `['readFile', path]` → text or `null` when missing (errors swallowed).
  - `['listdir', dir, flat?]` → `null` when `dir` does not exist, else `deepReadDir`: a flat array of absolute non-directory paths (`lstat`, so symlinks count as files), dot files/dirs included, no path normalisation on Windows. `['readdir', dir]` → `common-graph/readdir`: recursive, skips symlinks and every dot-prefixed name, but not `logseq/bak`.
  - `['writeFile', repo, path, content]`: `content` string or `ArrayBuffer` (→ `Buffer.from`), chmod 644 if not writable, `writeFileSync`, returns the stat. **Failures are caught**: a backup goes to `<repo>/logseq/bak/…`, the renderer gets an error notification, and the call resolves `nil`. `repo` is the graph directory. Parent directories are not created.
  - `['copyFile', repo, from, to]` → `fs-extra.copy` (creates the target's parents). `['unlink', repo, path]`: real `unlinkSync` only when `path` is under `~/.logseq` (`dotdir-file?`) or contains `assets/storages`; otherwise `renameSync` into `<repo>/logseq/.recycle/<rel with / → _>`, errors logged and swallowed. `['getLogseqDotDirRoot']` → `~/.logseq` (created if missing).
  - Watcher (`fs_watcher.cljs`): chokidar with `ignored = common-graph/ignored-path?` (dot-prefixed segments anywhere, `logseq/bak`, `logseq/.recycle`, `logseq/version-files`, `node_modules`, `.DS_Store`, `logseq/graphs-txid.edn`, `logseq/pages-metadata.edn`), `awaitWriteFinish: true`; `unlink` events are re-checked after 500 ms and dropped if the path exists again. Content is attached only for `.md .markdown .org .js .edn .css`.
